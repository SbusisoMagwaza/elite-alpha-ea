...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .confluence-checklist-item:last-child { border-bottom: none; }
        ...
        .confluence-check-text strong { color: #fff; }
        ...
        .balance-stat-value.profit { color: #00d4aa; }
        .balance-stat-value.loss { color: #ff6b6b; }
        ...
        .trade-symbol { color: #ccc; }
        .trade-pnl-win { color: #00d4aa; font-weight: 700; }
        .trade-pnl-loss { color: #ff6b6b; font-weight: 700; }
        .trade-time { color: #888; font-size: 11px; }
...
<body>
    ...
    <div class="tab-content active" id="home-tab">
        ...
        <div class="balance-tracker">
            ...
            <div class="balance-value" id="currentBalance">R364.00</div>

            <div class="balance-stats">
                <div class="balance-stat">
                    <div class="balance-stat-label">Today's P&L</div>
                    <div class="balance-stat-value" id="todayPnL">R0.00</div>
                </div>
                <div class="balance-stat">
                    <div class="balance-stat-label">Total Trades</div>
                    <div class="balance-stat-value" id="totalTrades">0</div>
                </div>
                <div class="balance-stat">
                    <div class="balance-stat-label">Win Rate</div>
                    <div class="balance-stat-value" id="balanceWinRate">0%</div>
                </div>
                <div class="balance-stat">
                    <div class="balance-stat-label">Total Profit</div>
                    <div class="balance-stat-value" id="totalProfit">R0.00</div>
                </div>
            </div>

            <div class="balance-buttons">
                <button class="balance-btn balance-btn-win" onclick="logTrade('WIN')">✓ WIN</button>
                <button class="balance-btn balance-btn-loss" onclick="logTrade('LOSS')">✗ LOSS</button>
            </div>

            <button onclick="resetBalance()" style="background: rgba(255, 107, 107, 0.1); color: #ff6b6b; border: 1px solid rgba(255, 107, 107, 0.3); padding: 8px; border-radius: 6px; font-size: 11px; margin-top: 10px; width: 100%; cursor: pointer;">🔄 Reset Tracker</button>
        </div>

        <!-- TRADE HISTORY -->
        <div class="trade-history" id="tradeHistorySection" style="display: none;">
            <div class="trade-history-title">📊 Recent Trades</div>
    ...
    <div class="tab-content" id="scan-tab">
        ...
        <div id="result">
            ...
            <div class="confidence-display">
                <div class="confidence-display-label">📊 Confidence</div>
                <div class="confidence-display-value" id="confidenceBig">85%</div>
                <span class="confidence-display-grade" id="confidenceGrade">Grade A+</span>
            </div>
    ...
    <script>
        ...
        function loadBalanceTracker() {
            const data = JSON.parse(localStorage.getItem('balanceTracker') || '{}');
            const balance = data.balance || 364.00;
            const trades = data.trades || [];

            document.getElementById('currentBalance').textContent = 'R' + balance.toFixed(2);
            document.getElementById('totalTrades').textContent = trades.length;

            // Today's P&L (last 24h)
            ...
            todayEl.className = 'balance-stat-value ' + (todayPnL >= 0 ? 'profit' : 'loss');

            // Total Profit
            const totalProfit = trades.reduce((sum, t) => sum + t.amount, 0);
            const totalEl = document.getElementById('totalProfit');
            totalEl.textContent = (totalProfit >= 0 ? '+' : '') + 'R' + totalProfit.toFixed(2);
            totalEl.className = 'balance-stat-value ' + (totalProfit >= 0 ? 'profit' : 'loss');

            // Win Rate
            const wins = trades.filter(t => t.amount > 0).length;
            const winRate = trades.length > 0 ? Math.round((wins / trades.length) * 100) : 0;
            document.getElementById('balanceWinRate').textContent = winRate + '%';
            ...
            if (trades.length > 0) {
                ...
                recent.forEach(t => {
                    ...
                    const timeStr = time.toLocaleString('en-ZA', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: 'short' });
                    const pnlClass = t.amount > 0 ? 'trade-pnl-win' : 'trade-pnl-loss';
                    const sign = t.amount > 0 ? '+' : '';
                    html += `<div class="trade-item">
                        <div>
                            <div class="trade-symbol">${t.symbol} ${t.type}</div>
                            <div class="trade-time">${timeStr}</div>
                        </div>
                        <div class="${pnlClass}">${sign}R${Math.abs(t.amount).toFixed(2)}</div>
                    </div>`;
                });
                document.getElementById('tradeHistoryList').innerHTML = html;
            } else {
        ...
        function logTrade(result) {
            ...
            const symbol = prompt('Which pair did you trade?\n(XAUUSDm, BTCUSDm, EURUSDm, etc.)', 'BTCUSDm') || 'Unknown';
            const type = prompt('Trade type?\n(BUY or SELL)', 'BUY') || 'BUY';

            // Calculate P&L (1% risk)
            ...
            data.balance = balance + pnl;
            data.trades = data.trades || [];
            data.trades.push(trade);

            localStorage.setItem('balanceTracker', JSON.stringify(data));

            // Update balance input on scan tab too
            if (document.getElementById('balance')) {
                document.getElementById('balance').value = data.balance.toFixed(2);
            }

            loadBalanceTracker();
            alert(`✅ Trade logged!\n\n${result}: ${pnl >= 0 ? '+' : ''}R${Math.abs(pnl).toFixed(2)}\nNew Balance: R${data.balance.toFixed(2)}`);
        }

        function resetBalance() {
            if (confirm('Reset balance tracker to R364.00? This will clear all trade history.')) {
                localStorage.removeItem('balanceTracker');
                loadBalanceTracker();
            }
        }
        ...
        function analyzeImage() {
            ...
            setTimeout(() => {
                ...
                document.getElementById('confidenceBig').textContent = confidence + '%';
                document.getElementById('confidenceGrade').textContent = 'Grade ' + grade;

                // CONFLUENCE CHECKLIST (Like Vertex Alpha/Generator X)
                const checklist = [
                    {name: 'H1 Accumulation Base', detail: 'Clear consolidation base established between key levels (inferred from M15 swing range)', pass: structure !== 'ranging'},
                    {name: 'Micro Manipulation Sweep', detail: 'Sharp Judas wick spiked above key highs, purging buy-side liquidity before immediately closing back inside', pass: liquidity},
                    {name: 'Distribution Expansion', detail: 'Displacement leg expanded with large-bodied bearish candles', pass: displacement},
                    {name: 'H1 Draw on Liquidity', detail: 'Draw on liquidity targets unmitigated sell-side lows around key zone', pass: bos},
                    {name: 'Displacement FVG / Imbalance', detail: 'Clear premium imbalance zone created during down-leg, now being tested as entry', pass: fvg},
                    {name: 'Entry Trigger on Retrace', detail: 'Price returned to retest the FVG breakdown level with upper wick rejection', pass: orderblock},
                    {name: 'R:Reward ≥ 2.0', detail: `Risk ${slDistance.toFixed(2)} for ${(tp - entry > 0 ? tp - entry : entry - tp).toFixed(2)} reward`, pass: true}
                ];
                ...
                checklist.forEach(item => {
                    ...
                    checklistHtml += `<div class="confluence-checklist-item">
                ...
                // DETAILED CONFLUENCES
                const confluences = [];
                if (liquidity) confluences.push('Clean buy-side liquidity grab at recent highs');
                if (displacement) confluences.push('M15 bearish displacement breaking structural lows');
                if (fvg) confluences.push('Deep premium retest into resistance / FVG');
                if (orderblock) confluences.push('High risk-to-reward ratio exceeding 2R');
                if (bos) confluences.push('Structure shift confirmed on H1 timeframe');
